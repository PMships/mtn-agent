...
const merchants_js_1 = require("./data/merchants.js");
const supabase = (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
const server = new index_js_1.Server({ name: "mtn-agent", version: "0.1.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    if (name === "search_merchants") {
        ...
        let results = merchants_js_1.MERCHANTS;
        if (category)
    ...
    if (name === "check_policy") {
        const { merchant_id, amount, category } = args;
        const policy = await getPolicy();
        const todaySpend = await getTodaySpend();
        const merchant = merchants_js_1.MERCHANTS.find((m) => m.id === merchant_id);
        if (!policy?.active) {
    ...
    if (name === "initiate_payment") {
        const { merchant_id, amount, description } = args;
        const merchant = merchants_js_1.MERCHANTS.find((m) => m.id === merchant_id);
        if (!merchant)
            throw new Error(`Merchant ${merchant_id} not found`);
        // Check policy before proceeding
        const policyCheck = await server.request({ method: "tools/call", params: { name: "check_policy", arguments: { merchant_id, amount, category: merchant.category } } }, {}).catch(() => null);
        const policy = await getPolicy();
        const todaySpend = await getTodaySpend();
        ...
        else if (!policy.allowed_categories.includes(merchant.category)) {
            decision = "REJECTED";
            reason = `Category not permitted`;
        }
        else if (amount > policy.single_transaction_limit) {
            decision = "REJECTED";
            reason = `Exceeds single transaction limit`;
        }
        else if (todaySpend + amount > policy.daily_limit) {
            decision = "REJECTED";
    ...
    if (name === "escalate_for_approval") {
        const { merchant_id, merchant_name, amount, reason } = args;
